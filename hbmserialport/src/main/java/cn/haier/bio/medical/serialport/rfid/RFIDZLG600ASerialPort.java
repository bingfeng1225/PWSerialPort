...
import cn.haier.bio.medical.serialport.rfid.tools.RFIDZLG600ATools;
...
public class RFIDZLG600ASerialPort implements PWSerialPortListener {
    ...
    private void write(int type) {
        byte[] data = RFIDZLG600ATools.packageCommand(type);
    ...
    public void onConnected(PWSerialPortHelper helper) {
        ...
        if(EmptyUtils.isNotEmpty(this.listener)){
            this.listener.get().onRFIDZLG600AConnected();
        }
    }
    ...
    public void onException(PWSerialPortHelper helper) {
        ...
        if (this.enabled && !"magton".equals(Build.MODEL)) {
            RFIDZLG600ATools.resetRFIDReader();
        }
    }
    ...
    public void onByteReceived(PWSerialPortHelper helper, byte[] buffer, int length) throws IOException {
        ...
        if (!this.ready) {
            ...
            if (mark == 0x06) {
                this.ready = true;
                if (EmptyUtils.isNotEmpty(this.listener)) {
                    this.listener.get().onRFIDZLG600AReady();
                }
        ...
        if (this.buffer.readableBytes() > 0) {
            ...
            this.buffer.resetReaderIndex();
            if (!RFIDZLG600ATools.checkFrame(data)) {
                return;
            }
    ...
    private void parseRFIDPackage() {
        ...
        this.buffer.discardReadBytes();
        if (EmptyUtils.isNotEmpty(this.listener)) {
            this.listener.get().onRFIDZLG600ARecognized(id, card);
        }
    }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    ...
                    write(RFIDZLG600ATools.RFID_COMMAND_UART);
                    break;
                case 1:
                    write(RFIDZLG600ATools.RFID_COMMAND_READ);
                    break;
            }
        }
    }
