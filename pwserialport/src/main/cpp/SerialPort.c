...
JNIEXPORT jobject JNICALL Java_cn_qd_peiwen_serialport_SerialPort_open
        (JNIEnv *env, jclass cls, jstring path, jint baudrate, jint stopBits, jint dataBits,
...
int mkdirs(const char *folder_path) {
    if (access(folder_path, 0) == 0) {                        /* 判断目标文件夹是否存在 */
        return 0;
    }
    char buffer[] = "mkdir -p ";
    char *cmd = alloca(strlen(buffer) + strlen(folder_path));
    memset(cmd, 0, strlen(cmd));
    sprintf(cmd, buffer, folder_path);
    LOGE("Cannot make dir cmd: %s", cmd);
...
JNIEXPORT void JNICALL Java_cn_qd_peiwen_serialport_SerialPort_writeFile
    ...
    const char *content_utf = (*env)->GetStringUTFChars(env, content, &iscopy);
    int len = write(fd, content_utf, strlen(content_utf));
    (*env)->ReleaseStringUTFChars(env, content, content_utf);
    if (len == -1) {
...
JNIEXPORT void JNICALL Java_cn_qd_peiwen_serialport_SerialPort_close
    ...
    jfieldID mFdID = (*env)->GetFieldID(env, SerialPortClass, "descriptor",
                                        "Ljava/io/FileDescriptor;");
    jfieldID descriptorID = (*env)->GetFieldID(env, FileDescriptorClass, "descriptor", "I");
    jobject mFd = (*env)->GetObjectField(env, thiz, mFdID);
    jint descriptor = (*env)->GetIntField(env, mFd, descriptorID);
    LOGD("Close serial prot(fd = %d)", descriptor);
    close(descriptor);
}
